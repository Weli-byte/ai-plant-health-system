...
export default function Dashboard() {
    ...
    try {
      ...
      for (const plant of userPlants) {
        try {
          ...
          // Bazı bitkilerin kaydı olmayabilir — sorun değil
        }
      }

      // 3. Son 5 analizi al ve formatlı hale getir
  ...
  // Sağlık dağılımını hesapla
  const distribution = calculateDistribution(recentAnalyses);
  const total = distribution.reduce((s, d) => s + d.value, 0) || 1;
  const healthyPct = plants.length > 0
    ? Math.round((recentAnalyses.filter((a) => a.severity === "ok").length / (recentAnalyses.length || 1)) * 100)
    : 76;
        ...
        <CardContent className="bg-leaf-gradient p-5 text-primary-foreground">
          ...
          <p className="mt-1 text-sm opacity-90">
            {plants.length > 0
              ...
              : "Tarlanın bugünkü durumu iyi görünüyor."}
          </p>
          <div className="mt-4 grid grid-cols-3 gap-2">
            <Stat icon={Sun} label="24°C" sub="Güneşli" />
            <Stat icon={Droplets} label="%58" sub="Nem" />
            <Stat icon={Wind} label="8 km/s" sub="Rüzgâr" />
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="rounded-3xl border-border/60 shadow-card">
          <CardContent className="p-4">
            ...
            <div className="mt-3 space-y-3">
              <FieldRow icon={Leaf} label="Aktif bitki" value={String(plants.length)} />
              <FieldRow icon={CheckCircle2} label="Sağlıklı" value={`%${healthyPct}`} tone="ok" />
              <FieldRow icon={TrendingUp} label="Bu hafta" value={`${recentAnalyses.length} analiz`} />
            </div>
          </CardContent>
        </Card>
      </div>
...
function calculateDistribution(analyses: RecentAnalysis[]) {
  if (analyses.length === 0) {
    return [
      { label: "Sağlıklı", value: 62, color: "hsl(96 38% 45%)" },
      { label: "Erken Uyarı", value: 24, color: "hsl(35 85% 55%)" },
      { label: "Hastalıklı", value: 14, color: "hsl(12 65% 52%)" },
    ];
  }
